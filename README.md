...
- LLM 隔离：复读由插件直接发送；成功复读后截断触发消息的常规 Command/LLM 流程。
...
- 支持文本、Unicode Emoji，以及数据完整且当前 NapCat/Host 可以重新发送的 QQ 表情。
- 命令、At、图片、回复引用、合并转发及其他富媒体消息不参与，并会结束当前队列。
- 成功复读后不再让触发消息进入常规 Command 或 LLM；没有抽中或发送失败时正常放行。
...
- NapCat 把部分 QQ 原生表情降级为文字时，插件只能按收到的文字复读；表情数据不完整时整条消息不参与。
